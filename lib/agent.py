    ...
    def __init__(self, model, policy="random", player=None):
        if type(policy) == str:
            if policy == "random":
                ...
                raise ValueError(
                    "The policy must be either 'random', 'one_step', or a custom policy."
                )
        if player is None:
            if model.is_game():
                player = model.player
...
class OnlineEvaluation(Agent):
    ...
    def get_values(self, states=None):
        ...
        if states is None:
            ...
            if not len(states):
                raise ValueError(
                    "The state space is too large. Please specify some states."
                )
    ...
    def get_best_actions(self, state):
        ...
        if len(actions) > 1:
            ...
            for action in actions:
                ...
                rewards = [
                    self.model.get_reward(next_state) for next_state in next_states
                ]
                next_values = self.get_values(next_states)
                # expected value
                value = np.sum(
                    np.array(probs)
                    * (np.array(rewards) + self.gamma * np.array(next_values))
                )
                values.append(value)
            if self.player == 1:
