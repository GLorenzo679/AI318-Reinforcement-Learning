...
from lib.display import display_board, display_position
    ...
    def reset(self, state=None):
        ...
        if state is None:
            ...
            if len(states):
                ...
                raise ValueError(
                    "Cannot set a random state. The state space is too large." ""
                )
        self.state = state
...
class Walk(Environment):
    ...
    def display(
        ...
        return display_position(
            image, self.state, states, marker, marker_size, marker_color, interval
        )
...
class Maze(Environment):
    ...
    def get_actions(state):
        ...
        if not Maze.is_terminal(state):
            ...
            actions = [
                action for action in all_actions if Maze.is_valid(state + action)
            ]
        return actions
    ...
    def display(
        self, states=None, marker="o", marker_size=200, marker_color="b", interval=200
    ):
        """Display the states."""
...
class Game(Environment):
    ...
    def show(self, state=None):
        ...
        for i in range(n_rows):
            rows.append(
                "|" + "".join([marker[board[i, j]] for j in range(n_cols)]) + "|"
            )
        print("\n".join(rows))
...
class TicTacToe(Game):
    ...
    def get_all_states(self):
        """Get all states."""
        boards = [
            np.array(board).reshape(TicTacToe.Board_Size)
            for board in itertools.product([-1, 0, 1], repeat=9)
        ]
...
class Nim(Game):
    ...
    def get_all_states(self):
        ...
        boards = [
            np.array(board)
            for board in itertools.product(*(np.arange(k + 1) for k in Nim.Board_Size))
        ]
        states = [(1, board) for board in boards] + [(-1, board) for board in boards]
        states = [state for state in states if self.is_valid(state)]
        return states
    ...
    def get_available_actions(self, state):
        ...
        if not self.is_terminal(state):
            ...
            actions = [
                (row, number + 1) for row in rows for number in range(board[row])
            ]
        return actions
    ...
    def display(
        ...
        return display_position(
            image, position, positions, marker, marker_size, marker_color, interval
        )
...
class ConnectFour(Game):
    ...
    def get_reward(state):
        ...
        sequence += "".join(
            [
                np.array2string(board.diagonal(offset=k), separator=sep)
                for k in range(-2, 4)
            ]
        )
        sequence += "".join(
            [
                np.array2string(np.fliplr(board).diagonal(offset=k), separator=sep)
                for k in range(-2, 4)
            ]
        )
